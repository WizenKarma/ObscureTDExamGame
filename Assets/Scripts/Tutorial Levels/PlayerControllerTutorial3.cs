...
public class PlayerControllerTutorial3: MonoBehaviour
{
    ...
    void SelectCombineTowers()
    {
        if (Input.GetKeyDown("" + 1))
        {
            ...
            tut.index++;
            tut.InGameText.text = tut.TutorialText[tut.index];
        }
        for (int i = 2; i <= 5; ++i)
        {
            if (Input.GetKeyDown("" + i))
    ...
    private void PlayerInput()
    {
        ...
        switch (gameManager.currentPhase)
        {
            ...
            case (PhaseBuilder.PhaseType.Tutorial4):
                {
                    NotLocked = false;
                    cam.transform.LookAt(FindObjectOfType<SphereCollider>().transform);
                    break;
                }
            ...
            case (PhaseBuilder.PhaseType.Tutorial6):
                {
    ...
    {
        if (collision.gameObject.layer== LayerMask.NameToLayer("GroundEnemy"))
        {
            //UnityEngine.SceneManagement.SceneManager.LoadScene("TutorialLevel3");
        }
    }
