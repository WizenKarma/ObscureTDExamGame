...
public class AstarAI : MonoBehaviour {
    ...
    public void Start() {
        ...
        targetPosition = this.GetComponent<Enemy>().firstWaypoint();
    ...
    private void pathHandler(){
        if (!searchingForPath) { // are we currently searching for a path?
            if (elapsedSinceRepath > repathRate) { // has enough time passed?
                seeker.StartPath(transform.position, targetPosition.position, OnPathComplete);
                searchingForPath = true;
